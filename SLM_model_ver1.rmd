...
all_sca <- c(15) # scale/s of the temporal response function
...
Clean up:
```{r}
rm(speed_df_long, p_speed_x, speed_df_x, p_speed_y, speed_df_y, p_speed_w, speed_df_w, 
   p_evidence_acc)
gc()
```
    ...
    for (this_condition_i in 1:length(unique(st$condition_ID))) {
      ...
      res <- foreach(iter_i = 1:n_iterations, .verbose = FALSE, .errorhandling = "stop",
                     ...
                     .combine = 'rbind' ) %dofun% {
                       ...
                       # instead, check whether data frames are identical (they should be)
      ...
      res_agg <- cbind(res_agg, this_readout)
...
save.image(file = file.path("visual_proc_on_SLMF_rev_sca", sca, ".rda"), compress = "xz")
...
First, let's apply some aethetics...

```{r}
# merge with st to get the absolute velocity
...
p_O <- ggplot(data = all_res, aes(x = t, y = O_full-O_absent, color = velFac_f)) + 
  #geom_hline(yintercept = 0.1, linetype = "dashed", size = 0.75) + 
  geom_ribbon(aes(x = t, ymax = (O_full-O_absent)+O_full_SD, ymin = (O_full-O_absent)-O_full_SD, fill = velFac_f), 
              alpha = 0.3, color = NA) + 
  geom_line(size = 1.2) + 
  geom_hline(yintercept = 0, linetype = "solid", size = 1) + 
  scale_color_viridis_d(option = "viridis") + 
  scale_fill_viridis_d(option = "viridis") + 
  facet_grid(mask_time_f~visual_scale_f+amp_f) + 
  theme_classic(base_size = 12) + SlomTheme() + coord_cartesian(expand = FALSE, ylim = c(0, 0.9)) + 
...
p_prob_sum <- ggplot(data = prob_sum_agg, aes(x = velFac, y = prob_sum, 
  ...
  geom_hline(yintercept = k_threshold, linetype = "dashed") + 
  ...
  facet_grid(mask_time_f~visual_scale_f+fa) + 
  theme_classic(base_size = 12) + SlomTheme() + #coord_cartesian(expand = FALSE) + 
  #scale_y_sqrt(breaks = round(seq(sqrt(0.5), sqrt(20), length.out = 6)^2,1), expand = c(0,0)) + 
  scale_y_log10() + 
  scale_x_log10() +
  labs(x = "Velocity [% Peak Vel.]", y = TeX("$(\\sum_{t}^{} |O_{static+moving}(t)\ -\ O_{static}(t)|^{\\beta})^{\\frac{1}{\\beta}}$"), 
       color = "Amplitude [dva]", fill = "Amplitude [dva]")
p_prob_sum

# some evidence readouts
prob_sum_agg[ , log_prob_sum := log(prob_sum)]
...
sim_p_correct_expand <- expand.grid(list(mask_time_f = unique(sim_p_correct$mask_time_f), 
                                         ...
                                         velFac = seq(min(sim_p_correct$velFac), max(sim_p_correct$velFac), 0.01) ))
sim_p_correct_expand$glm_predict <- predict(glm_sim_p_correct, type = "response", newdata = sim_p_correct_expand)
sim_p_correct_expand$fa <- "Response"
...
# plot the results of the simulation
p_sim_p_correct_abs <- ggplot(data = sim_p_correct, aes(x = abs_vel, y = m / n_trials, 
                                                    color = amp_f, group = amp_f)) + 
  geom_hline(yintercept = 0.5, linetype = "dashed") + 
  geom_vline(xintercept = 1, linetype = "dashed") + 
  # geom_errorbar(data = sim_p_correct, aes(x = velFac, ymax = m_upper / n_trials, 
...
setDT(prob_sum_agg)
prob_sum_agg[ , min_evi := min(prob_sum), by = .(visual_scale_f)]
sim_p_correct_nothres <- prob_sum_agg[ , c(binom_sampler_nothres(e_mean = prob_sum, e_sd = 4, k = 0)), # previously 5
                                       by = .(mask_time_f, visual_scale_f, amp_f, velFac_f, velFac)]
sim_p_correct_nothres$n_correct <- round(sim_p_correct_nothres$m)
sim_p_correct_nothres$fa <- "Response"
sim_p_correct_nothres$prop_correct <- sim_p_correct_nothres$m / sim_p_correct_nothres$n_trials
# fit psychometric functions to no-threshold data
...
sim_p_correct_nothres_expand <- expand.grid(list(mask_time_f = unique(sim_p_correct_nothres$mask_time_f), 
                                                 ...
                                                 velFac = seq(min(sim_p_correct_nothres$velFac), 
                                                              max(sim_p_correct_nothres$velFac), 0.01) ))
...
sim_p_correct_nothres_expand$fa <- "Response"
...
p_position_signal_x <- ggplot(data = all_res[t>t_start+lat+10], aes(x = t+150, y = x_full, color = velFac_f)) + 
  #geom_point(data = all_stim, aes(x = t-150, y = x, color = velFac_f), alpha = 1, size = 0.25) + # the stimulus
  geom_ribbon(aes(x = t+150, ymax = x_full+x_full_SD, ymin = x_full-x_full_SD, fill = velFac_f), alpha = 0.3, color = NA) + # perceived SD
  geom_line(size = 1.2) + # perceived
  #geom_line(data = all_res[t>t_start+lat+10], aes(x = t, y = y_full, color = velFac_f), linetype = "dashed") + 
  scale_color_viridis_d(option = "viridis") + 
  scale_fill_viridis_d(option = "viridis") + 
  facet_grid(mask_time_f~visual_scale_f+amp_f) + 
  theme_classic(base_size = 12) + SlomTheme() + coord_cartesian(expand = FALSE) + 
...
p_position_signal_y <- ggplot(data = all_res[t>t_start+lat+10], aes(x = t+150, y = y_full, color = velFac_f)) + 
  ...
  #geom_point(data = all_stim, aes(x = t-150, y = y, color = velFac_f), alpha = 1, size = 0.25) + # the stimulus
  geom_ribbon(aes(x = t+150, ymax = y_full+y_full_SD, ymin = y_full-y_full_SD, fill = velFac_f), alpha = 0.3, color = NA) + # perceived SD
  geom_line(size = 1.2) + # perceived
  #geom_line(data = all_res[t>t_start+lat+10], aes(x = t, y = y_full, color = velFac_f), linetype = "dashed") + 
  scale_color_viridis_d(option = "viridis") + 
  scale_fill_viridis_d(option = "viridis") + 
